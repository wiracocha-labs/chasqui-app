<template>
  ...
  <div
    class="main-container flex flex-row h-screen overflow-hidden bg-bg-primary"
  >
    ...
    <div class="flex flex-row flex-1 ml-20">
      ...
      <div
        class="middle-sidebar w-[260px] min-w-[260px] flex flex-col border-r border-brand-20 bg-terciary px-3 py-4 shadow-md"
      >
        <div class="flex flex-col gap-4">
          ...
          <div class="mb-6 flex items-center gap-3">
            <!-- Profile Avatar Box -->
            <div
              class="w-12 h-12 rounded-lg bg-slate-700 flex-shrink-0 border border-brand-20 flex items-center justify-center overflow-hidden"
            >
              <img
                src="../assets/images/chasqui_avatar1.webp"
                alt="Emilio Gutiérrez"
                class="w-full h-full object-cover"
              />
            </div>
            <div class="flex-auto">
              <h1
                class="font-semibold text-lg leading-tight truncate text-white"
              >
                Emilio Gutiérrez
              </h1>
          ...
          <div class="mb-6 bg-brand-10 text-white rounded-xl">
            <div class="flex justify-between items-center px-3 pb-2 text-white">
              ...
              >
                <svg
                  class="w-4 h-4 fill-secondary group-hover:fill-brand transition-colors"
            ...
            <div
              v-if="userGroups.length === 0 && !isLoading"
          ...
          <div class="mb-6 bg-brand-10 text-white rounded-xl">
            ...
            >
              ...
              <span class="text-white">{{
                getDirectConversationName(direct)
              }}</span>
            </div>

            <div
              v-if="userDirects.length === 0 && !isLoading"
              class="px-3 py-2 text-xs text-white/40 italic"
            >
          ...
          <div class="text-white rounded-xl">
            <div
              class="flex justify-between items-center px-3 pb-2 text-white bg-brand-10 rounded-xl"
            >
      ...
      <div class="flex-1 flex flex-col h-screen overflow-hidden bg-bg-primary">
        <div
          ...
          class="border-b border-bg-primary border-opacity-10 p-4 flex justify-between items-center"
        >
          <div>
            <h2 class="text-xl font-bold flex items-center gap-2">
              ...
              {{
                ...
                getDirectConversationName(currentConversation)
              }}
            </h2>
            <div class="flex gap-3 text-xs mt-1">
              ...
              <span
                :class="isSocketConnected ? 'text-green-500' : 'text-red-500'"
                class="flex items-center gap-1"
              >
                <span
                  class="w-2 h-2 rounded-full"
          ...
          <div class="flex items-center gap-3">
            <button @click="showInviteModal = true" class="btn-tertiary">
              <svg
                class="w-4 h-4"
                viewBox="0 0 24 24"
                fill="none"
        ...
        <!-- Chat messages -->
        <div
          class="px-4 py-4 flex-1 overflow-y-scroll message-container bg-bg-primary"
        ...
        >
          ...
          <div class="space-y-6">
            <div
              v-if="!currentConversation"
              class="h-full flex flex-col items-center justify-center mt-20 opacity-50"
            >
            ...
            >
              <div class="text-2xl mb-2">👋</div>
              <h3 class="text-base font-semibold mb-1 text-color-brand">
                No hay mensajes aún
              </h3>
              <p class="text-xs text-color-textSecondary">
            ...
            >
              <div
                class="w-12 h-12 rounded-xl bg-slate-700 flex-shrink-0 border-2 border-brand-20 overflow-hidden flex items-center justify-center"
              >
                <div class="text-xl">👤</div>
              </div>
              <div class="flex-1">
                <div class="flex items-baseline gap-2 mb-1">
                  <span class="font-bold text-white text-[15px]">{{
                    message.sender
                  }}</span>
                  <span class="text-[12px] opacity-40">{{
        ...
        <div class="pb-6 px-4 flex-none">
          <form
            ...
            class="flex rounded-lg border-2 overflow-hidden border-accent bg-accent-10"
          >
            <input
      ...
      >
        <div
          class="bg-primary-light border border-white border-opacity-10 rounded-2xl w-full max-w-md p-6 shadow-2xl"
        >
          <h3 class="text-xl font-bold mb-4">Invitar colaborador</h3>
          <p class="text-sm opacity-70 mb-6">
            Agrega a alguien a esta conversación usando su correo electrónico.
          </p>

          <div class="space-y-4">
            <div>
              <label class="block text-sm font-medium opacity-60 mb-1"
      ...
      >
        <div
          class="bg-terciary w-[400px] rounded-2xl border border-brand-20 p-6 shadow-2xl"
        >
          ...
          <div class="space-y-4">
            <div v-if="modalType === 'group'">
              <label
                class="block text-xs font-semibold text-color-textSecondary mb-1 uppercase"
            ...
            <div>
              ...
              <p
                ...
                class="text-[10px] text-color-textSecondary mt-1 italic"
              >
                Separa las direcciones por comas.
              </p>
            </div>
          </div>
          ...
          <div class="flex gap-3 mt-6">
            ...
            <button
              ...
              :disabled="
                modalType === 'group' ? !newGroupName : !newGroupParticipants
              "
...
const loadConversations = async () => {
  ...
  try {
    ...
    if (!Array.isArray(convs)) {
      log.error(
        "ChatView",
        "Invalid conversations response (expected array):",
        convs,
      );
...
const trySelectFromUrl = async (id: string) => {
  ...
  log.info(
    "ChatView",
    `Trying to auto-select conversation from URL ID: ${idToFind}`,
  );
  const allConvs = [...userGroups.value, ...userDirects.value];

  log.debug(
    "ChatView",
    `Iterating over ${allConvs.length} available conversations to find ID`,
  );
  ...
  if (found) {
    ...
    log.warn(
      "ChatView",
      `No conversation found for URL ID: ${idToFind} among ${allConvs.length} conversations. Available IDs: ${allConvs.map(getConversationId).join(", ")}`,
    );
  }
};
...
watch(
  () => [userGroups.value, userDirects.value],
  () => {
    if (!currentConversation.value && route.params.id) {
      log.info(
        "ChatView",
        "Conversations updated, re-evaluating URL selection...",
      );
...
watch(
  [isSocketConnected, currentConversation],
  ([connected, conv]) => {
    if (connected && conv) {
      const fullId = getStrId(conv.id);
      ...
      log.info(
        "ChatView",
        `🔄 Auto-joining room: ${backendId} (Socket=${connected})`,
      );
...
const createConversation = async () => {
  ...
  try {
    ...
    log.debug(
      "ChatView",
      `Payload a enviar para crear conversación: ${JSON.stringify(payload)}`,
    );
...
const handleInviteGuest = async () => {
  ...
  try {
    ...
    log.info(
      "ChatView",
      `Adding participant ${inviteEmail.value} to ${fullId}`,
    );

    const { apiPost } = await import("../services/api");
    await apiPost(
      `/conversations/${fullId}/participants`,
      payload,
      authStore.token,
    );
    ...
    alert(
      "Error al agregar al usuario. Asegúrate de que el usuario exista o la wallet sea válida.",
    );
  }
};
...
const sendMessage = async () => {
  ...
  try {
    ...
    if (isSocketConnected.value) {
      ...
      log.error(
        "ChatView",
        "❌ Socket not connected, message might not be persisted",
      );
      alert(
        "Error: La conexión en tiempo real está desactivada. Reintenta en unos segundos.",
      );
    }
  } catch (err) {
...
const selectConversation = async (conv: Conversation) => {
  ...
  log.info(
    "ChatView",
    `Selecting conversation. Short: ${shortId}, Full: ${fullId}, Backend: ${backendId}`,
  );
  ...
  try {
    ...
    const msgs = await apiGet<any[]>(
      `/conversations/${fullId}/messages`,
      authStore.token,
    );
  ...
  (newId) => {
    if (newId) {
      ...
      if (
        found &&
        (!currentConversation.value ||
          getConversationId(currentConversation.value) !== newId)
      ) {
...
onMounted(async () => {
  try {
    ...
    if (route.query.target_wallet) {
      ...
      log.info(
        "ChatView",
        `Direct chat requested from query param for wallet: ${wallet}`,
      );

      // Try to find if we already have a direct chat with this person
      const existingDirect = [...userGroups.value, ...userDirects.value].find(
        (c) =>
          c.participants.some((p) =>
            getStrId(p).toLowerCase().includes(wallet.toLowerCase()),
          ),
      );
      ...
      if (existingDirect) {
        ...
        const { apiPost } = await import("../services/api");
        try {
          const newConv = await apiPost<Conversation>(
            "/conversations",
            {
          ...
          log.error(
            "ChatView",
            "Error creating direct chat from query param",
            err,
          );
        }
      }
    ...
    onEvent((event) => {
      ...
      if (event.type === "NewMessage" && event.message) {
        ...
        const cleanSenderAddr = senderIdStr.includes(":")
          ? senderIdStr.split(":")[1]
          : senderIdStr;

        const isFromMe =
          cleanSenderAddr.toLowerCase() ===
            currentMatchId.value?.toLowerCase() ||
          (authStore.address &&
            cleanSenderAddr.toLowerCase() === authStore.address.toLowerCase());

        log.info(
          "ChatView",
          `👤 Sender: ${cleanSenderAddr}. MyID: ${currentMatchId.value}. IsFromMe? ${isFromMe}`,
        );

        if (!isFromMe) {
          const currentFullId = currentConversation.value
            ? getStrId(currentConversation.value.id)
            : null;
          const currentBackendId = currentFullId
            ? getBackendId(currentFullId)
            : null;
          const messageConvId = getStrId(msg.conversation_id || "");

          if (
            currentBackendId === messageConvId ||
            (currentFullId && currentFullId === messageConvId)
          ) {
            log.info(
              "ChatView",
              "✅ Appending message from another user to UI",
            );
            messages.value.push({
              text: msg.content,
            ...
            log.info(
              "ChatView",
              "⏳ Message is for another conversation, ignoring.",
            );
          }
        } else {
          log.info(
            "ChatView",
            "🙈 Message is from me, ignoring (already added locally).",
          );
        }
      }
    });
